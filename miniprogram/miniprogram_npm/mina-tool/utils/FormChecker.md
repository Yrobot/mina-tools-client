...
// 创建实例对象
this.FormChecker = new FormChecker(rules, messages)

// 自定义验证规则
this.FormChecker.addMethod('assistance', (value, param) => {
    return this.FormChecker.optional(value) || (value.length >= 1 && value.length <= 2)
}, '请勾选1-2个敲码助手')
...
submitForm(e) {
    ...
    // 传入表单数据，调用验证方法
    if (!this.FormChecker.checkForm(e)) {
        const error = this.FormChecker.errorList[0]
        return false
    }
},
```